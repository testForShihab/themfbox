...
class MfuBankInfo extends StatefulWidget {
  const MfuBankInfo({super.key});

  @override
  State<MfuBankInfo> createState() => _MfuBankInfoState();
}

class _MfuBankInfoState extends State<MfuBankInfo> {
  ...
  void fillData() {
    ...
    bankProof = bankInfo['bank_proof_desc'];
    bankProofCode = bankInfo['bank_proof'];
  }
  ...
  Widget uploadContentTile(BuildContext context) {
    if (bse_nse_mfu_flag != 'BSE') return SizedBox();
  ...
  List bankProofList = [
    {"desc": "Latest Bank Passbook", "code": "14"},
    {"desc": "Latest Bank Account Statement", "code": "15"},
    {"desc": "Cheque Copy", "code": "77"},
    {"desc": "Bank Letter", "code": "78"}
  ];
  String bankProof = "Latest Bank Passbook";
  String bankProofCode = "14";
  ExpansionTileController bankProofController = ExpansionTileController();

  Widget bankProofTile(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
          color: Colors.white, borderRadius: BorderRadius.circular(10)),
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          subtitle: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(bankProof, style: AppFonts.f50012),
            ],
          ),
          children: [
            ListView.builder(
              shrinkWrap: true,
              physics: NeverScrollableScrollPhysics(),
              itemCount: bankProofList.length,
              itemBuilder: (context, index) {
                Map map = bankProofList[index];
