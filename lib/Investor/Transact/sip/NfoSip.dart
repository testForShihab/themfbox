...
class NfoSip extends StatefulWidget {
  const NfoSip({super.key});

  @override
  State<NfoSip> createState() => _NfoSipState();
}

class _NfoSipState extends State<NfoSip> {
  ...
  bool isLoading = true;
  List<MyLumpsumFundsPojo> sipPojoList = [];

  Future getDatas() async {
    isLoading = true;
    // EasyLoading.show();

    await getNfoSipSchemes();
  ...
  Future getNfoSipSchemes() async {
    if (sipPojoList.isNotEmpty) return 0;

    Map data = await TransactionApi.getNfoSipSchemes(
  ...
  convertListToObj(List list) {
    ...
    for (var element in list) {
      sipPojoList.add(MyLumpsumFundsPojo.fromJson(element));
    }
  }
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        return Scaffold(
          ...
          appBar: invAppBar(
            title: Utils.getFirst13("NFO SIP", count: 18),
          ),
          body: SideBar(
            child: Column(
              ...
              children: [
                topFilterArea(),
                (!isLoading && sipPojoList.isEmpty)
                    ? Center(child: NoData())
                    : Expanded(
                        child: ListView.builder(
                          shrinkWrap: true,
                          itemCount: isLoading ? 5 : (sipPojoList.length),
