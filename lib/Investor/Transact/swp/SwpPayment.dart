...
import 'package:mymfbox2_0/Investor/Transact/swp/SwpPaymentSuccess.dart';
...
class SwpPayment extends StatefulWidget {
  const SwpPayment({super.key});

  @override
  State<SwpPayment> createState() => _SwpPaymentState();
}

class _SwpPaymentState extends State<SwpPayment> {
  ...
  Future makePayment() async {
    ...
    Map data = await TransactionApi.multipleSwp(
      user_id: user_id,
      client_name: client_name,
      broker_code: arn,
      euin_code: euin,
      ...
      bse_nse_mfu_flag: client_code_map['bse_nse_mfu_flag'],
      firstOrderFlag: (client_code_map['bse_nse_mfu_flag'].toUpperCase() == 'BSE') ? first_order_flag : " ",
      payment_type: '',
      payment_mode: '',
    );
    ...
    return FutureBuilder(
        ...
        builder: (context, snapshot) {
          return Scaffold(
            ...
            extendBody: true,
            appBar: invAppBar(title: "SWP Order Details"),
  ...
  String firstPayment = "SWP with first payment";
  List firstPaymentList = [
    "SWP with first payment",
    "SWP without first payment",
  ];
