...
class SchemeInfoController extends GetxController {
  ...
  Future<int> getSchemeInfo({
    ...
    Map<String, dynamic> data = await ResearchApi.getSchemeInfo(
      user_id: userId,
      client_name: clientName,
    ...
    schemeInfoPojo.value = SchemeInfoPojo.fromJson(data);
    historicalTableData.value =
        List<Map<String, dynamic>>.from(data['scheme_performance_list']);
    peerComparisionTableData.value =
        List<Map<String, dynamic>>.from(data['peerComparisonResponse']);

    assetClass.value = data['asset_class'] ?? "";
    schemeInceptionDate.value = data['scheme_inception_date'] ?? "";
    category.value = data['scheme_category'];
    scheme_status.value = data['scheme_status'];
    ...
    minLumpsum.value = (data['minimum_investment'] ?? 0).toInt();
    minSip.value = data['schemeMapping']['sip_minimum_amount'] ?? 0;
    aum.value = data['scheme_assets'] ?? 0;
    assetDate.value = data['scheme_asset_date'] ?? "";
    ter.value = data['schemeMapping']['ter'] ?? 0;
    schemeTurnover.value = data['scheme_turnover'] ?? 0;
    schemeBenchmark.value = data['scheme_benchmark'] ?? "";
    exit1.value = data['schemeMapping']['exit1'] ?? "";
    schemeObjective.value = data['schemeMapping']['scheme_objective'] ?? "";
    schemeManager.value = data['scheme_manager'] ?? "";
    factsheetName.value = data['factsheet_name'] ?? "";
    portfolioName.value = data['portfolio_name'] ?? "";
    factsheetLink.value = data['factsheet_link'] ?? "";
    portfolioLink.value = data['portfolio_link'] ?? "";
    amcDetails.value = data['amc_details'] ?? "";
    amcName.value = data['amc_details']['amc_name'] ?? "";
    riskometerImage.value = data['riskometer_image'] ?? "";
    amcLogo.value = data['amc_logo'] ?? "";
    amcAum.value = data['amc_details']['aum'] ?? "0";

    finalAmcAum.value = double.parse(amcAum.value.replaceAll(',', ''));
    aumShare.value = data['amc_details']['aum_share'] ?? "";
    activeFunds.value = data['amc_details']['active_funds'] ?? "0";
    amcStartDate.value = convertDateFormat(data['amc_details']['start_date']);
    amcAddress1.value = data['amc_details']['address1'];
    amcAddress2.value = data['amc_details']['address2'];
    amcAddress3.value = data['amc_details']['address3'];
    city.value = data['amc_details']['city'];
    pincode.value = data['amc_details']['pincode'];
    phone.value = data['amc_details']['phone'];

    schemeMap.value = data['schemeMapping'];
    creditRateList.value = data['credit_ratings'];

    marketLargeCap.value =
        data['schemeMapping']['market_cap_largecap_percent'] ?? 0;
    marketMidCap.value =
        data['schemeMapping']['market_cap_midcap_percent'] ?? 0;
    marketSmallCap.value =
        data['schemeMapping']['market_cap_smallcap_percent'] ?? 0;
