...
class AllSubBroker extends StatefulWidget {
  const AllSubBroker({super.key});

  @override
  State<AllSubBroker> createState() => _AllSubBrokerState();
}

class _AllSubBrokerState extends State<AllSubBroker> {
  ...
  List rmList = [];
  Future getAllSubbrokerDetails() async {
    if (rmList.isNotEmpty) return 0;

    Map data = await AdminApi.getAllSubbrokerDetails(
    ...
    return FutureBuilder(
        ...
        builder: (context,snapshot){
          return Scaffold(
            ...
            appBar: adminAppBar(title: "All Associate", bgColor: Colors.white),
            body: Column(
              children: [
                Container(
                  color: Colors.white,
                ...
                countLine(),
                (isLoading) ? Utils.shimmerWidget(devHeight * 0.6) : listArea(),
              ],
            ),
          );
        });
