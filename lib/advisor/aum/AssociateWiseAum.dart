...
class AssociateWiseAum extends StatefulWidget {
  const AssociateWiseAum({super.key});

  @override
  State<AssociateWiseAum> createState() => _AssociateWiseAumState();
}

class _AssociateWiseAumState extends State<AssociateWiseAum> {
  int user_id = GetStorage().read("mfd_id") ?? 0;
  String client_name = GetStorage().read("client_name") ?? "null";

  Future fetchChartData(String sub_broker_name) async {
    EasyLoading.show();
    Map data = await Api.getAumHistoryByMonths(
      ...
      type: "sub_broker",
      type_user_name: sub_broker_name,
    );
        ...
        child: SingleChildScrollView(
            ...
            child: (isLoading)
                ...
                itemBuilder: (context, index) {
                  ...
                  String title = data['subbroker_name'];
                  num amount = data['net_brokerage'];
                  String branch = data['branch'];
                  String rm = data['rm_name'];
  ...
  detailsBottomSheet(Map data, List<ChartData> chartData) {
    double sheetHeight = devHeight * 0.3;
    bool showChart = false;
    String name = data['subbroker_name'];
    num amount = data['net_brokerage'];
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
